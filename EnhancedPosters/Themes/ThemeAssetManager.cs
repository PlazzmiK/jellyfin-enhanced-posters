...
namespace EnhancedPosters.Themes;
...
public class ThemeAssetManager
{
    ...
    /// <summary>
    /// Gets the base path for Enhanced Posters themes.
    /// </summary>
    public string ThemesDirectory
    {
        get
        {
            var newDir = Path.Combine(_applicationPaths.PluginConfigurationsPath, "EnhancedPosters", "themes");
            var oldDir = Path.Combine(_applicationPaths.PluginConfigurationsPath, "PostersEnhanced", "themes");
    ...
    public SKBitmap? GetBadge(string theme, string badgeKey, float targetHeight, string? customBadgeDataUrl = null, bool transparentBg = false)
    {
        ...
        var candidateDiskPaths = new[]
        {
            Path.Combine(ThemesDirectory, theme, "badges", $"{badgeKey}.png"),
            Path.Combine(ThemesDirectory, theme, "badges", "resolution", $"{badgeKey}.png"),
            Path.Combine(ThemesDirectory, theme, "badges", "edition", $"{badgeKey}.png"),
            Path.Combine(ThemesDirectory, theme, "badges", "audio", $"{badgeKey}.png"),
        };

        foreach (var customPath in candidateDiskPaths)
        {
        ...
        // 3. Check embedded assembly resources (Kometa resolution, edition, audio, and 3D badges)
        try
        {
            using var resStream = GetBadgeResourceStream(badgeKey);
    ...
    private static Stream? GetBadgeResourceStream(string badgeKey)
    {
        var key = badgeKey.ToLowerInvariant();
        var assembly = typeof(ThemeAssetManager).Assembly;

        var candidates = new List<string>();
        void AddKeyCandidates(string k)
        {
            candidates.Add($"EnhancedPosters.Assets.Badges.resolution.{k}.png");
            candidates.Add($"EnhancedPosters.Assets.Badges.edition.{k}.png");
            candidates.Add($"EnhancedPosters.Assets.Badges.audio.{k}.png");
            candidates.Add($"EnhancedPosters.Assets.Badges.{k}.png");
        }

        AddKeyCandidates(key);

        switch (key)
        {
            case "directorscut":
                AddKeyCandidates("directors");
                break;
            case "specialedition":
                AddKeyCandidates("special");
                break;
            case "finalcut":
                AddKeyCandidates("final");
                break;
            case "hdr10plus":
                AddKeyCandidates("plus");
                break;
            case "hdr10":
            ...
            case "dtshd":
                ...
                break;
            case "sd":
                AddKeyCandidates("480p");
                AddKeyCandidates("576p");
                break;
            case "atmos":
                AddKeyCandidates("dolby_atmos");
                break;
        }
